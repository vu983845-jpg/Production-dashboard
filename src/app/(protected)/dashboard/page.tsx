...
export default function DashboardPage() {
    ...
    // ── React Query — single hook replaces ~650 lines of raw fetching ────────
    const { data: qData, isLoading: queryLoading } = useDashboardData(selectedMonth)

    const dashboardsData = qData?.dashboardsData ?? {}
    const energyHistory = qData?.energyHistory ?? []
    const shellingElecHistory = qData?.shellingElecHistory ?? []
    const compressorHistory = qData?.compressorHistory ?? []
    const compressorMtd = qData?.compressorMtd ?? { total: 0, m1: 0, m2: 0, m3: 0 }
    const otherElecSummary = qData?.otherElecSummary ?? { shellingKwh: 0, compressorKwh: 0, peelingCompKwh: 0, woodKg: 0 }
    const otherElecMtd = qData?.otherElecMtd ?? {}
    const dailyElecVsProd = qData?.dailyElecVsProd ?? []
    const kpiSummary = qData?.kpiSummary ?? {
        steamActual: 0, steamTarget: 0, fgwhActual: 0, fgwhTarget: 0,
        contActual: 0, contTarget: 0, elecActual: 0, elecTarget: 0,
        waterActual: 0, waterTarget: 0, woodActual: 0, woodTarget: 0,
    ...
    const dailyRecords = useMemo(() => {
        if (!qData || selectedDept === 'all') return []
        return (qData.dailyRecords || []).filter((d: any) => d.department_id === selectedDept)
    }, [qData, selectedDept])

    const pageLoading = queryLoading

    // ── One-time: load user profile for greeting (cached in sessionStorage) ──
    useEffect(() => {
        const cached = sessionStorage.getItem('vicc_dept_code')
        const cachedName = sessionStorage.getItem('vicc_user_name')
        if (cached) {
            setUserProfile({ deptCode: cached, userName: cachedName || '' })
            return
        }
        async function loadProfile() {
            ...
            if (p?.department_id) {
                ...
                setUserProfile({ deptCode, userName: p.full_name || "" })
                sessionStorage.setItem('vicc_dept_code', deptCode)
                sessionStorage.setItem('vicc_user_name', p.full_name || '')
            }
        }
        loadProfile()
    }, [])

    // Load Departments (small, fast, cached independently)
    useEffect(() => {
                        ...
                        <Card className="mt-3 md:mt-4 bg-white border border-slate-200 shadow-sm relative overflow-hidden">
                            ...
                            <CardHeader className="p-3 pb-2 md:p-5 md:pb-3 bg-white/40 border-b border-white/50 relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
                                ...
                                <div className="flex flex-wrap items-center gap-4 md:gap-6 bg-white/50 px-4 py-2 rounded-lg border border-white/80 shadow-sm">
                                    ...
                                    <Link href="/input?tab=water" className="flex flex-col items-start md:items-end hover:bg-blue-50/50 p-1 -m-1 rounded transition-colors group">
                                        <span className="text-[10px] uppercase text-muted-foreground font-bold tracking-tight group-hover:text-blue-500">Nước (m³) 🔗</span>
                                ...
                                <div className="md:hidden flex flex-col gap-10 py-4">
                                    ...
                                    <div className="h-48 w-full">
                                        <div className="flex justify-between items-center mb-2">
                                            <p className="text-xs font-bold text-blue-600 uppercase tracking-tight">Water Chart (m³)</p>
                                            <Link href="/input?tab=water" className="text-[10px] bg-blue-50 text-blue-600 px-2 py-1 rounded-md font-bold shadow-sm border border-blue-100 flex items-center gap-1 active:scale-95 transition-transform">
                                                Nhập liệu ↗
                                            </Link>
                                        </div>
                                        <ResponsiveContainer width="100%" height="100%">
