-- 1. Thêm cột isp_ton vào bảng daily_actual
ALTER TABLE public.daily_actual ADD COLUMN IF NOT EXISTS isp_ton numeric(10,3) DEFAULT 0;

-- 2. Drop views cũ
DROP VIEW IF EXISTS public.v_dashboard_total_daily CASCADE;
DROP VIEW IF EXISTS public.v_dashboard_daily CASCADE;

-- 3. Tạo lại v_dashboard_daily với đầy đủ cột (container, điện, isp_ton)
...
-- 4. Tạo lại v_dashboard_total_daily với đầy đủ FGWH và Container
