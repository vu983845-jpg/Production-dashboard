...
export function ShellingReportTab({ data }: ShellingReportTabProps) {
    ...
    const productionLabel = data.totals.achievementPct >= 100 ? "đạt kế hoạch" : "chưa đạt kế hoạch"
    const stabilityNote = data.weekly.length > 1
        ? `Tuần yếu nhất là ${[...data.weekly].filter((row) => row.plan > 0).sort((a, b) => a.achievementPct - b.achievementPct)[0]?.name ?? "N/A"}; cần ép cứng downtime để tránh đánh giá chỉ theo tổng tháng.`
        : "Chưa đủ dữ liệu tuần để đánh giá độ ổn định trong tháng."

    const executiveNotes = [
        `Shelling ${productionLabel}: ${fmt(data.totals.actual, 1)} T actual / ${fmt(data.totals.plan, 1)} T plan, chênh ${planGap >= 0 ? "+" : ""}${fmt(planGap, 1)} T.`,
        `Năng suất bình quân đạt ${fmt(data.totals.productivityTph, 3)} T/h với ${fmt(data.totals.hours, 1)} giờ chạy line và ${fmt(data.totals.tonPerMan, 2)} T/man-shift.`,
        `Broken bình quân có trọng số là ${fmt(data.totals.brokenPct, 2)}%, ${data.totals.brokenPct > 5.5 ? "vượt" : "dưới"} ngưỡng mục tiêu 5,5%.`,
        topDowntimeCause ? `Downtime lớn nhất thuộc nhóm ${topDowntimeCause.name}, chiếm ${fmt(topDowntimeCause.share, 1)}% tổng downtime event.` : "Chưa có downtime event trong kỳ báo cáo.",
        stabilityNote,
    ]

    const actionPlan = [
        highestDowntimeLine ? `Khóa nguyên nhân downtime line ${highestDowntimeLine.name}: daily review theo root cause, ca, máy/khu vực; mục tiêu giảm ít nhất 15-20% phút dừng trong tháng kế tiếp.` : "Chuẩn hóa ghi nhận downtime theo line/ca để xác định bottleneck chính xác hơn.",
        highestBrokenLine ? `Mở containment chất lượng cho line ${highestBrokenLine.name}: kiểm tra size mix, dao/cỡ máy, độ ẩm nguyên liệu ở ca có alert broken cao.` : "Duy trì kiểm soát broken theo từng line có sản lượng đáng kể.",
        weakestLine ? `Tách bài toán năng suất line ${weakestLine.name}: so sánh setup, manpower và thời gian chờ với line benchmark ${bestLine?.name ?? "tốt nhất"}.` : "Bổ sung dữ liệu line để phân tích năng suất theo bottleneck.",
        topDowntimeCause ? `Với nhóm ${topDowntimeCause.name}, lập Pareto 3 nguyên nhân con và owner hành động trong họp sản xuất hàng ngày.` : "Thiết lập Pareto downtime chuẩn trước khi phân công hành động.",
    ]
    ...
    return (
        <div className="space-y-4 pb-8">
                ...
                <div className="bg-slate-950 px-5 py-5 text-white">
                    <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
                        ...
                        <div className="grid grid-cols-3 gap-2 text-right">
                            <div className="rounded-lg border border-white/10 bg-white/10 px-3 py-2"><p className="text-[10px] font-bold uppercase text-slate-300">Ngày SX</p><p className="text-xl font-black">{data.coverage.activeDays}</p></div>
                            <div className="rounded-lg border border-white/10 bg-white/10 px-3 py-2"><p className="text-[10px] font-bold uppercase text-slate-300">Dừng line</p><p className="text-xl font-black">{data.coverage.lineRows}</p></div>
            ...
            <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
                <ReportStat label="Actual / Plan" value={`${fmt(data.totals.actual, 1)} T`} sub={`${fmt(data.totals.achievementPct, 1)}% kế hoạch`} icon={Target} tone={achievementTone} />
                <ReportStat label="Productivity" value={`${fmt(data.totals.productivityTph, 3)} T/h`} sub={`${fmt(data.totals.hours, 1)} giờ chạy`} icon={Gauge} tone="blue" />
                <ReportStat label="Downtime" value={hours(data.totals.downtimeMin)} sub={`${fmt(data.totals.downtimeMin, 0)} phút event`} icon={Clock} tone="red" />
                <ReportStat label="Broken" value={`${fmt(data.totals.brokenPct, 2)}%`} sub="Mục tiêu kiểm soát 5,5%" icon={AlertTriangle} tone={brokenTone} />
            ...
            <section className="grid gap-4 xl:grid-cols-[1.15fr_0.85fr]">
                <Card className="rounded-xl border-white/70 bg-white/85 py-0 shadow-sm"><CardContent className="p-5">
                    ...
                    <h4 className="mt-1 text-lg font-black text-slate-900">Nhận định điều hành tháng</h4>
                ...
                <Card className="rounded-xl border-white/70 bg-white/85 py-0 shadow-sm"><CardContent className="p-5">
                    ...
                    <h4 className="mt-1 text-lg font-black text-slate-900">Độ phủ dữ liệu</h4>
                    ...
                    <p className="mt-3 text-xs font-semibold text-slate-500">Ghi chú: số liệu production ưu tiên daily actual; năng suất, broken, manpower và downtime line lấy từ dữ liệu line/ca.</p>
                </CardContent></Card>
            </section>
            ...
            {comparison && (
                <section className="rounded-xl border border-white/70 bg-white/85 p-4 shadow-sm">
                    <div className="mb-3 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                        <div><p className="text-xs font-black uppercase tracking-widest text-slate-400">Month over month</p><h4 className="text-lg font-black text-slate-900">So với tháng trước</h4></div>
            ...
            <section className="grid gap-4 xl:grid-cols-[1.5fr_1fr]">
                ...
                <Card className="rounded-xl border-white/70 bg-white/85 py-0 shadow-sm"><CardContent className="p-4">
                    <p className="text-xs font-black uppercase tracking-widest text-slate-400">Weekly cadence</p><h4 className="mb-3 text-lg font-black text-slate-900">Tiến độ theo tuần</h4>
