...
function TabAnalysisInner({ summaries, historical, currentMonth, lang: externalLang }: Props) {
    ...
    const avgActualWater2025 = useMemo<number | null>(() => {
        const vals = allMonths
            .filter(m => m.startsWith('2025'))
            .map(m => histMap[m]?.[5]?.actual_energy ?? null)
            .filter((v): v is number => v != null)
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 6 }}>
                        {ALL_SEU_IDS.map(id => {
                            ...
                            if (id === 5) {
                                ...
                                const rcnKg = h?.rcn_hap_duoc_kg ?? null
                                const waterPiPerTon = enpi != null ? enpi * 1000 : null
                                const referencePerTon = ref != null ? ref * 1000 : null
                                const actualWater = actual != null ? Number(actual) : null
                                const savedM3 = enpi != null && ref != null && rcnKg != null ? (ref - enpi) * rcnKg : null
                                const isSaving = savedM3 != null && savedM3 >= 0
                                return <div key={id} className="rounded-lg flex flex-col overflow-hidden" style={{ background: '#FFFFFF', border: `1.5px solid ${cfg.border}`, boxShadow: '0 1px 3px rgba(142,30,25,0.07)' }}>
                                    <div className="flex items-center justify-between px-2 pt-1.5 pb-1" style={{ borderBottom: `1.5px solid ${cfg.color}18` }}><div className="flex items-center gap-1.5"><div style={{ width: 18, height: 18, borderRadius: 5, background: cfg.color + '18', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><cfg.Icon className="h-2.5 w-2.5" style={{ color: cfg.color }} /></div><span style={{ fontSize: 10, fontWeight: 800, color: '#1E293B' }}>WATER</span></div>{delta != null ? <span style={{ fontSize: 8.5, fontWeight: 800, padding: '1px 5px', borderRadius: 4, background: delta > 0 ? '#FEE2E2' : '#D1FAE5', color: delta > 0 ? '#991B1B' : '#065F46' }}>{delta > 0 ? 'UP' : 'DOWN'} {Math.abs(delta).toFixed(1)}%</span> : <span style={{ fontSize: 7.5, color: '#94A3B8' }}>NO REF</span>}</div>
                                    <div className="px-2 pt-1 rounded-md" style={{ background: '#ECFDF5', border: '1px solid #A7F3D0' }}><div style={{ fontSize: 6.5, color: '#047857', fontWeight: 900, letterSpacing: '0.05em' }}>WATER USED THIS MONTH</div><div style={{ fontSize: 18, fontWeight: 900, color: '#047857', lineHeight: 1.05, fontVariantNumeric: 'tabular-nums', marginTop: 1 }}>{actualWater != null ? Math.round(actualWater).toLocaleString('vi-VN') : '-' } <span style={{ fontSize: 8 }}>M3</span></div><div style={{ fontSize: 7, color: '#0F766E', marginTop: 2, fontWeight: 700 }}>WATER PI: {waterPiPerTon != null ? waterPiPerTon.toFixed(3) : '-'} M3 / TON RCN</div></div>
                                    <div className="px-2 pb-1"><div className="flex items-center justify-between" style={{ fontSize: 7, marginTop: 3, gap: 4 }}><div style={{ flex: 1 }}><div style={{ color: '#94A3B8', fontSize: 6, fontWeight: 600 }}>CURRENT</div><div style={{ fontWeight: 800, color: '#334155', fontSize: 8.5 }}>{waterPiPerTon != null ? waterPiPerTon.toFixed(3) : '-'}</div></div><div style={{ flex: 1 }}><div style={{ color: '#94A3B8', fontSize: 6, fontWeight: 600 }}>REF 2025</div><div style={{ fontWeight: 700, color: '#64748B', fontSize: 8.5 }}>{referencePerTon != null ? referencePerTon.toFixed(3) : '-'}</div></div></div><div style={{ fontSize: 6.5, color: '#64748B', marginTop: 3 }}>WATER {actualWater != null ? Math.round(actualWater).toLocaleString('vi-VN') : '-'} M3 / RCN {rcnKg != null ? (rcnKg / 1000).toLocaleString('vi-VN', { maximumFractionDigits: 3 }) : '-'} TON</div>{savedM3 != null && <div className="mt-1 rounded px-1.5 py-0.5" style={{ background: isSaving ? '#D1FAE520' : '#FEE2E220', border: `1px solid ${isSaving ? '#10B98140' : '#EF444440'}` }}><span style={{ fontSize: 8, fontWeight: 900, color: isSaving ? '#059669' : '#DC2626' }}>{isSaving ? 'SAVING' : 'OVER'} {Math.abs(Math.round(savedM3)).toLocaleString('vi-VN')} M3</span></div>}</div>
                                </div>
                            }
