-- 1. Thêm cột target điện vào daily_plan
ALTER TABLE public.daily_plan ADD COLUMN IF NOT EXISTS target_electricity_kwh NUMERIC DEFAULT 0;

-- 2. Thêm cột chỉ số đồng hồ điện vào daily_kpi
ALTER TABLE public.daily_kpi ADD COLUMN IF NOT EXISTS electricity_meter_reading NUMERIC;

-- 3. Cập nhật View v_dashboard_daily để lấy thêm chỉ số điện và tự tính tiêu thụ
...
SELECT
    ...
    COALESCE(k.electricity_meter_reading - k.prev_electricity_meter_reading, 0) as electricity_consumption_kwh,
    p.target_broken_pct,
    p.target_unpeel_pct,
    p.target_sw_pct,
    p.target_isp_pct,
...
-- 4. Tạo lại View v_dashboard_total_daily
