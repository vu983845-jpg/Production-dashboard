...
export async function POST(request: Request) {
    try {
        ...
        // Extract the token_hash from the action link generated by admin
        const url = new URL(linkData.properties.action_link)
        const token_hash = url.searchParams.get('token_hash')

        if (!token_hash) {
            return NextResponse.json(
                { error: 'Token hash missing from generated link' },
                { status: 500 }
            )
        }

        // Exchange the token hash for a session
        const { error: verifyError } = await adminSupabase.auth.verifyOtp({
            type: 'magiclink',
            token_hash,
        })

        if (verifyError) {
            console.error('Error verifying generated session:', verifyError)
            return NextResponse.json(
