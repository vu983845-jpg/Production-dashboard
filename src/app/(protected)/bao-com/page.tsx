                                                                                                ...
                                                                                                onClick={async () => {
                                                                                                    ...
                                                                                                    for (const [date, draftVal] of Object.entries(rowEditDrafts)) {
                                                                                                        ...
                                                                                                        if (!rec) {
                                                                                                            ...
                                                                                                            if (!deptIdForInsert) { errors.push(date + ': missing dept'); continue }
                                                                                                            const res = await fetch('/api/meal-headcount', {
                                                                                                                method: 'POST',
                                                                                                                headers: { 'Content-Type': 'application/json' },
                                                                                                                body: JSON.stringify({
                                                                                                                    ...
                                                                                                                    vegetarian: 0, ot_count: 0, ot_vegetarian: 0,
                                                                                                                    note: null,
                                                                                                                })
                                                                                                            })
                                                                                                            ...
                                                                                                            if (res.ok) {
                                                                                                                ...
                                                                                                                if (saved?.id) setStatsData(prev => prev ? [...prev.filter(r => r.id !== saved.id), saved] : [saved])
                                                                                                            } else errors.push(date + ': ' + (json.error || res.statusText))
                                                                                                            continue
                                                                                                        }
                                                                                                        // PATCH existing record; guard with rec.department_name
                                                                                                        const diff = newVal - orig
                                                                                                        const oldOff = rec.official_present ?? 0
                                                                                                        const oldSea = rec.seasonal_present ?? 0
                                                                                                        let newOff = oldOff + diff
                                                                                                        let newSea = oldSea
                                                                                                        if (newOff < 0) { newSea = Math.max(0, oldSea + newOff); newOff = 0 }
                                                                                                        const newVeg = Math.min(rec.vegetarian ?? 0, newOff + newSea)
                                                                                                        const res = await fetch('/api/meal-headcount', {
                                                                                                            method: 'PATCH',
                                                                                                            headers: { 'Content-Type': 'application/json' },
                                                                                                            body: JSON.stringify({
                                                                                                                id: rec.id,
                                                                                                                official_present: newOff,
                                                                                                                seasonal_present: newSea,
                                                                                                                vegetarian: newVeg,
                                                                                                        ...
                                                                                                        if (res.ok) {
                                                                                                            savedCount++
                                                                                                            setStatsData(prev => prev ? prev.map(r => r.id === rec.id ? { ...r, official_present: newOff, seasonal_present: newSea, vegetarian: newVeg } : r) : prev)
                                                                                                        } else {
                                                                                                            const j = await res.json().catch(() => ({}))
                                                                                                            errors.push(date + ': ' + (j.error || res.statusText))
                                                                                                        }
                                                                                                    }
                                                                                                    if (savedCount > 0) { setEditingRowKey(null); setRowEditDrafts({}) }
                                                                                                    if (errors.length > 0) alert('Save errors:\n' + errors.join('\n'))
                                                                                                    setRowSaving(false)
                                                                                                ...
                                                                                                onClick={async () => {
                                                                                                    ...
                                                                                                    for (const [date, draftVal] of Object.entries(rowEditDrafts)) {
                                                                                                        ...
                                                                                                        if (res.ok) {
                                                                                                            ...
                                                                                                            const j = await res.json().catch(() => ({}))
                                                                                                            otErrors.push(date + ': ' + (j.error || res.statusText))
                                                                                                        }
                                                                                                    }
                                                                                                    if (otSavedCount > 0) { setEditingRowKey(null); setRowEditDrafts({}) }
                                                                                                    if (otErrors.length > 0) alert('OT save errors:\n' + otErrors.join('\n'))
                                                                                                    setRowSaving(false)
                                                                                                    setEditingRowKey(null)
                                                                                                    setRowEditDrafts({})
                                                                                                }}
