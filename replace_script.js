...
let content = fs.readFileSync('C:/Users/Cashew/.gemini/PPE/factory-dashboard/src/app/(protected)/input/page.tsx', 'utf8');

const regex1 = /\s*\)\s*:\s*departments\.find\(d\s*=>\s*d\.id\s*===\s*selectedDept\)\?\.code\s*===\s*'PEEL_MC'\s*\?\s*\(\s*<>\s*<TableRow>\s*<TableCell\s*className="font-medium\s*align-middle\s*text-blue-700">\s*Pass\s*1\s*—\s*Tổng\s*3\s*ca\s*\(Tấn\)\s*<\/TableCell>.*?<\/>\s*\)\s*:/s;

const replacement1 = `                                                                  ) : departments.find(d => d.id === selectedDept)?.code === 'PEEL_MC' ? (    
                                                                      <>    
...
const regex2 = /\s*\{\/\*\s*──\s*Peeling\s*3-shift\s*breakdown\s*card\s*──\s*\*\/\}.*?\{\/\*\s*──\s*Color\s*Sorter\s*2-shift\s*breakdown\s*card\s*──\s*\*\/\}/s;
...
fs.writeFileSync('C:/Users/Cashew/.gemini/PPE/factory-dashboard/src/app/(protected)/input/page.tsx', content);
