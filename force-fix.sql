...
-- 1. Force basic schemas and table permissions
GRANT USAGE ON SCHEMA public TO anon, authenticated, service_role;
GRANT ALL ON public.daily_plan TO anon, authenticated, service_role;
GRANT ALL ON public.daily_actual TO anon, authenticated, service_role;
GRANT ALL ON public.daily_kpi TO anon, authenticated, service_role;
GRANT ALL ON public.departments TO anon, authenticated, service_role;
GRANT ALL ON public.profiles TO anon, authenticated, service_role;
GRANT ALL ON public.daily_containers TO anon, authenticated, service_role;

-- 2. Drop corrupted views
DROP VIEW IF EXISTS public.v_dashboard_total_daily CASCADE;
DROP VIEW IF EXISTS public.v_dashboard_daily CASCADE;
DROP VIEW IF EXISTS public.v_dashboard_base CASCADE;

-- 3. Rebuild base view (Include Containers in date ranges)
CREATE OR REPLACE VIEW public.v_dashboard_base AS
SELECT
    d.id as department_id,
    dates.work_date
FROM public.departments d
CROSS JOIN (
    SELECT work_date FROM public.daily_plan
    UNION SELECT work_date FROM public.daily_actual
    UNION SELECT work_date FROM public.daily_kpi
    UNION SELECT work_date FROM public.daily_containers
) dates;

-- 4. Rebuild daily view
...
-- 6. Explicitly grant VIEW permissions
GRANT SELECT ON public.v_dashboard_base TO anon, authenticated, service_role;
GRANT SELECT ON public.v_dashboard_daily TO anon, authenticated, service_role;
GRANT SELECT ON public.v_dashboard_total_daily TO anon, authenticated, service_role;

COMMIT;

-- 7. Hard-reset PostgREST API schema cache
